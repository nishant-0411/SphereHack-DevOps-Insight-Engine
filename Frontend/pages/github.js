...
export default function GitHubPage() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3005'}/api/github/analyze`, {
        method: "POST",
  ...
  const handleFix = async () => {
    ...
    setFixing(true);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3005'}/api/github/fix`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          token,
